...
// var registrationModule = angular.module('angularjs-dropdown-multiselect', []);

registrationModule.directive('mfDropdownStaticInclude', ['$compile', function ($compile) {
...
registrationModule.directive('ngDropdownMultiselect', ['$filter', '$document', '$compile', '$parse', function ($filter, $document, $compile, $parse) {
	return {
		restrict: 'AE',
		scope: {
			selectedModel: '=',
		...
		template: function (element, attrs) {
			...
			template += '<div class="input-group"> <input type="text" style="width: 270px;" class="form-control" ng-model="textSelect" readonly> <div class="input-group-btn"><button ng-disabled="disabled" type="button" class="dropdown-toggle" ng-class="settings.buttonClasses" ng-click="toggleDropdown()">{{getButtonText()}}<span class="glyphicon glyphicon-menu-down" aria-hidden="true"></span></button></div></div>';
		...
		link: function ($scope, $element, $attrs) {
			...
			$scope.texts = {
				...
				buttonDefaultText: 'Seleciona las opciones',
				...
				if ($scope.settings.dynamicTitle && ($scope.selectedModel.length > 0 || (angular.isObject($scope.selectedModel) && Object.keys($scope.selectedModel).length > 0))) {
					if ($scope.settings.smartButtonMaxItems > 0) {
						...
						if ($scope.singleSelection) {
							...
							if ($scope.selectedModel.length > 3) {
								totalSelected = 'Varios elementos seleccionados.'
							} else {
								$scope.selectedModel.forEach(value => {
									totalSelected += `${value.label}, `;
								});
							};
						}

						if (totalSelected === 0) {
							$scope.textSelect = $scope.texts.buttonDefaultText;
							// return $scope.texts.buttonDefaultText;
						} else {
							$scope.textSelect = totalSelected.substring(0, totalSelected.length - 2);
							// return totalSelected.substring(0, totalSelected.length - 2);
						}
					}
				} else {
					$scope.textSelect = $scope.texts.buttonDefaultText;
